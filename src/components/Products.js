...
export default function Products() {
  ...
  const [imageUrl, setImageUrl] = useState(""); // Yeni eklenen görsel state'i
  ...
  async function urunEkle() {
    ...
    try {
      await addProduct({
        ...
        imageUrl: imageUrl.trim() || null // Görsel URL eklendi
      });
      // Formu temizle
  ...
  function duzenlemeAc(p) {
    setEditing({ 
      ...
      stock: p.stock || 0, 
      category: p.category || "",
      imageUrl: p.imageUrl || "" // Düzenleme için görseli çek
    });
  }
  ...
  async function duzenlemeKaydet() {
    ...
    try {
      await updateProduct(id, {
        ...
        imageUrl: img ? String(img).trim() : null // Görsel URL güncelleme
      });
      setEditing(null);
      await yukle();
          ...
          <div className="product-list">
            {filtered.map(p => (
              <div key={p.id} className="product-item">
                {/* Görsel Gösterimi */}
                {p.imageUrl && (
                  <div className="prod-img-wrapper" style={{ marginRight: '12px', flexShrink: 0 }}>
                    <img 
                      src={p.imageUrl} 
                      alt={p.name} 
                      style={{ width: '45px', height: '45px', borderRadius: '6px', objectFit: 'cover', border: '1px solid #eee' }} 
                      onError={(e) => { e.target.style.display = 'none'; }} // Hata verirse gizle
                    />
                  </div>
                )}
        ...
        <div className="modal-overlay">
          <div className="modal-card">
            ...
            <div className="modal-body">
              ...
              {/* Düzenleme Modalında Görsel URL */}
              <div className="form-group">
                <label>Görsel URL</label>
                <div className="input-icon-wrapper">
                    <FiImage className="input-icon" style={{top: '50%', transform: 'translateY(-50%)'}} />
                    <input 
